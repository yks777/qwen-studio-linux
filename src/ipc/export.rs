...
use crate::config::schema::{ChatMessage, ChatExport};
...
pub async fn export_chat(
    ...
    let export = ChatExport { title: title.clone(), messages, exported_at: now };

    let (content, default_name, filter_name, filter_ext) = match format.as_str() {
        "json" => (export.to_json()?, format!("{}.json", sanitize(&title)), "JSON", "json"),
        "html" => (export.to_html(), format!("{}.html", sanitize(&title)), "HTML", "html"),
        _ => (export.to_markdown(), format!("{}.md", sanitize(&title)), "Markdown", "md"),
    };

    let (tx, rx) = std::sync::mpsc::channel();
    app.dialog().file().set_title("Export Chat")
...
impl ChatExport {
    ...
    pub fn to_html(&self) -> String {
        format!(r#"<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{}</title>
        <style>body{{font-family:sans-serif;max-width:800px;margin:auto;padding:20px;background:#1a1a1a;color:#e0e0e0;}}</style>
            ...
            self.title, self.title, self.exported_at,
            self.messages.iter().map(|m| {
                let (cls, lbl) = match m.role.as_str() {
                    "user" => ("user", "You"),
                    _ => ("assistant", "Qwen"),
                };
                format!("<div class='{}'><b>{}</b><pre>{}</pre></div>",
                    cls, lbl, html_escape(&m.content))
            }).collect::<String>()
        )
    }
